...
MooSelect.implement({Implements:[Options,Events],options:{zIndex:1000,performAutoTabIndexing:true,tabIndex:null,className:'container',classPrefix:'MooSelect-',globalClassName:null,globalClassPrefix:null,selectFirstOnDefault:true,allowOtherResult:false,allowDeselectSingle:false,customBuildResultHTML:null,customBuildStageResultHTML:null,animations:true,fireSelectEvents:true,disableSearcher:false,messages:{noResults:Locale.get('MooSelect.messages.noResults'),otherResult:Locale.get('MooSelect.messages.otherResult')}},initialize:function(element,options){element=$(element);element.store('MooSelect',this);this.input=element;this.multiple=this.input.get('multiple');if(this.isMultiple()){this.input.set('multiple','multiple');var name=this.input.get('name');if(name&&!name.test(/\[\]$/)){this.input.set('name',name+'[]');}}
...
if(result){result.select();}},scrollToSelected:function(result){var selected=result||this.getSelected();if(selected){this.getScroller().scrollToElement($(selected));}},scrollToHoverResult:function(result){var hover=result||this.getHoverResult();if(hover){this.getScroller().toElement($(hover));}},select:function(result){result.select();},onSelect:function(result){this.blurAll(result);this.deSelectAll(result);var text=result.getText();var value=result.getValue();var index=$(result).retrieve('index');result.hover();this.hoverIndex=index;this.fireEvent('select',[text,value]);},destroy:function(){this.getContainer().destroy();}});})(document.id);(function($){MooSelect.Remote=new Class({Extends:MooSelect,options:{filterResults:false,hideResultsWhenSearching:true,mergeResults:false,cache:true,minSearchLength:3,messages:{loading:Locale.get('MooSelect.messages.loading'),minSearch:Locale.get('MooSelect.messages.minSearch')}},initialize:function(element,options){options=options||{};options.disableSearcher=false;options.remoteOptions=options.remoteOptions||{};if(options.url){options.remoteOptions.url=options.url;delete options.url;}
...
this.parent(value);}});})(document.id);(function($){MooSelect.Results.Remote=new Class({Extends:MooSelect.Results,options:{url:null,requestSearchParam:'q',requestClass:Request.JSON,requestOptions:{method:'GET'},requestResponseHandler:null,cache:true},initialize:function(options){this.parent(options);this._filter=this.filter.bind(this);this.filter=this.$filter.bind(this);this.clearCache();if(this.options.preloadSearch!=null){this.filter(this.options.preloadSearch,true);}},$filter:function(search,skipLength){if(!this._filter){this._filter=this.parent.bind(this);}
